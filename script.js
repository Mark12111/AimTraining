...
//функция начала игры
function startGame() {
    //через секунду после выбора времени начать его отсчет   
    setInterval(decreaseTime, 1000);
    //создаем первый кружок
    createRandomCircle();
    //передаем время, которое выбрали для игры
    setTime(time);
}
...
function decreaseTime() {
    ...
    if (time === 0) {
        finishGame(); //то завершаем игру
    }
    else {
        //если время не равно нулю
        let current = --time; //то уменьшить время на 1
        if (current < 10) { //если число внутри current меньше 10
            current = `0${current}`; //тогда current начинает отсчет
        }
        //поместить в функцию отображение времени на чигру
        setTime(current);
    }
}

//функция для добавления и отражения времени
